...
namespace
{
    ...
    TEST_F(ReflectAPI_MetaMachine_Tests, setMachineName)
    {
        char name[] = "Test Name";
        refl_initMetaMachine(&metaMachine);
        ASSERT_EQ(refl_setMetaMachineName(metaMachine, name), API_SUCCESS) << "Expecting success" << std::endl;
        ASSERT_NE(refl_setMetaMachineName(metaMachine, NULL), API_SUCCESS) << "Expecting failure for null value" << std::endl;

        // Retrieving and checking name value
        char buffer[20];
        CLReflectResult res = refl_getMetaMachineName(metaMachine, buffer, 20);
        ASSERT_EQ(res, API_SUCCESS) << "Expecting successful name retrieval" << std::endl;
    ...
    TEST_F(ReflectAPI_MetaMachine_Tests, bufferOverFlowTest)
    {
        ...
        ASSERT_EQ(res, API_BUFFER_OVERFLOW) << "Expecting buffer overflow since buffer is too small" << std::endl;
    }
