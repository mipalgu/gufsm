...
class CPP_MetaFileWriter(CLMetaGenWriter):
    ...
    INCLUDES = ['#include "CLReflectAPI.h"', '#include <sstream>', '#include <iostream>', '#include <cstdlib>', '#include <cstring>', '#include <stdint.h>', '#include <gufsm/clfsm/clfsm_machine_loader.h>', '#include <gufsm/gufsm/FSMachine.h>']
    ...
    def writeHeader(self, cpp):
        ...
        with cpp.block('extern "C"'):
            cpp("static unsigned int machineID;")
            cpp("refl_metaMachine Create_MetaMachine();")
            cpp("refl_metaMachine Create_ScheduledMetaMachine();")
        cpp('')
